...
public class CalGrid extends JFrame implements ActionListener {
	...
	public CalGrid(ApptStorageControllerImpl con) {
		...
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				if(mCurrUser.getDeleted()==1){
					mCurrUser.setDeleted(2);
					controller.updateUser(mCurrUser);
				}
				System.exit(0);
			}
		});
	...
	JMenuBar createMenuBar() {
		...
		JMenu Userbar = (JMenu) menuBar.add(new JMenu("User"));
		Userbar.setMnemonic('U');
		Userbar.getAccessibleContext().setAccessibleDescription("User info editor");
		JMenuItem ui;
		ui = (JMenuItem) Userbar.add(new JMenuItem("edit"));
		ui.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				if(mCurrUser.getAdmin()==0){
					UserDialog info=new UserDialog("userinfo",controller);
				}else {
					AdminDialog ad=new AdminDialog(controller);
				}
			}
		});
		
		ui = (JMenuItem) Userbar.add(new JMenuItem("info"));
		ui.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				String s;
				if(mCurrUser.getAdmin()==0){
					...
					JOptionPane.showMessageDialog(getParent(), 
							"UserID: " + mCurrUser.getID()+
                            "\nfull name: " +controller.getDefaultUser().getFullname() +
                            "\nEmail: " +controller.getDefaultUser().getAddress()+
                            "\nUser class: "+s
                            ,
		...
		mi.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent arg0) {
				if(controller.getDefaultUser().getAdmin()==1){
					...
					JOptionPane.showMessageDialog(null, "Only admin can modify locations",
		...
		Access = (JMenu) menuBar.add(new JMenu("Help"));
        Access.setMnemonic('h');
        mi = (JMenuItem) Access.add(new JMenuItem("About"));
        mi.setMnemonic('a');
        mi.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                        JOptionPane.showMessageDialog(getParent(), "COMP3111 Project Group 47\n" +
                                        "Version 1.0\n" +
                                        "\n" +
                                        "Developers:\n" +
                                        "Chan Tsz Kin \n" +
        ...
        Appmenu.add(new JPopupMenu.Separator());
        mi = new JMenuItem("Go to today ");
        mi.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                        goToDay(today.get(Calendar.YEAR),today.get(today.MONTH),today.get(today.DAY_OF_MONTH));
                        tableView.grabFocus();
                }
        });
        Appmenu.add(mi);
	...
	private void initializeSystem() {
		...
		File f = new File("appointment.xml");
		if(f.exists()){
			controller.LoadApptFromXml();
			int size = ApptStorageMemImpl.mAppts.size();
			if (size!=0){
				Appt[] todaylist = new Appt[size];
				for(int i = 0;i<size;i++){
					todaylist[i] = new Appt();
					todaylist[i] = (Appt) ApptStorageMemImpl.mAppts.get(i+1);
					controller.ManageAppt(todaylist[i], 4);
				}
				applist.clear();
				applist.setTodayAppt(todaylist);
			}
		}
		if(mCurrUser.getDeleted()==1){
			JOptionPane.showMessageDialog(this, "Your account has been deleted by the Admin\nAll data will be destroy after you logout.");
		}
		
		//checkUpdateJoinAppt();
		Thread t1 = new Thread(new ReminderCheck(mCurrUser, GetTodayAppt()), "Reminder");
		t1.start();
		Thread t2 = new Thread(new GroupApptCheck());
		t2.start();
	}
...
//	}
	...
	public Vector<Appt> GetThisDayAppt() {
        Iterator it = this.controller.getApptMap().values().iterator();
        Vector<Appt> thisdayApptVector= new Vector<Appt>();
        while (it.hasNext()) {
                Appt apptTmp = (Appt) it.next();
                //currentY + "-" + currentM                     + "-" + currentD
                Timestamp tstart=apptTmp.TimeSpan().StartTime();
                int yearTmp=tstart.getYear();
                yearTmp+=1900;
                int monthTmp= tstart.getMonth();
                monthTmp+=1;
                int dayTmp=tstart.getDate();
                boolean matchOrNot= (yearTmp==currentY)&&(monthTmp==currentM)&&(dayTmp==currentD);
                if (matchOrNot) {
                        thisdayApptVector.add(apptTmp);
                }
        }
        return thisdayApptVector;
	}
	
	public Vector<Appt> GetThisDayAppt(int cyear, int cmonth, int cday) {
        Iterator it = this.controller.getApptMap().values().iterator();
        Vector<Appt> thisdayApptVector= new Vector<Appt>();
        while (it.hasNext()) {
                Appt apptTmp = (Appt) it.next();
                ...
                monthTmp+=1;
                int dayTmp=tstart.getDate();
                boolean matchOrNot= (yearTmp==cyear)&&(monthTmp==cmonth)&&(dayTmp==cday);
                if (matchOrNot) {
                        thisdayApptVector.add(apptTmp);
                }
        }
	...
	public void updateMarkersBasedOnApptMarkerArray() {
        if (mCurrUser != null) {
                mCurrTitle = "Desktop Calendar - " + mCurrUser.getID() + " - ";
                this.setTitle(mCurrTitle + "(" + currentY + "-" + currentM + "-"
                                + currentD + ")");
                ...
                this.setTitle("Desktop Calendar - No User - ("
                                + currentY + "-" + currentM + "-" + currentD + ")");
        }
	}
	
	public void goToDay(int goYear, int goMonth, int goDay) {
        currentY = goYear;
        year.setText(new Integer(currentY).toString());

        month.setSelectedIndex(goMonth);
        currentD = goDay;
        updateMarkersBasedOnApptMarkerArray();
	}
