...
public class CalGrid extends JFrame implements ActionListener {
	...
	public CalGrid(ApptStorageControllerImpl con) {
		...
		tableView = new JTable(dataModel) {
			public TableCellRenderer getCellRenderer(int row, int col) {
				...
				if(apptsForToday!=null&&apptsForToday.length!=0)
					System.out.println("NumberOfAppointmentsForToday = " + apptsForToday.length);
				if (tem.equals("") == false) {
					try {
	...
	private Appt[] getApptsForDay(String day) {
		...
		Appt[] tempAppt2 = null;
		if(!day.equals("")){
			Timestamp morning = new Timestamp(Integer.parseInt(year.getText()), month.getSelectedIndex(), Integer.parseInt(day), 8, 0, 0, 0);
			//System.out.println(morning.toString());
			Timestamp evening = new Timestamp(Integer.parseInt(year.getText()), month.getSelectedIndex(), Integer.parseInt(day), 17, 45, 0, 0);
