...
public class TimeSpan implements Serializable {
	...
	public boolean Overlap(TimeSpan d) {
		if (d.EndTime().before(mStartTime) || d.EndTime().equals(mStartTime)){	// If the time span ends before or at the starting time of this time span then these two time spans do not overlap
			//System.out.println("Does not overlap(1)");
			return false;
		}
		...
		if (d.StartTime().equals(mEndTime) || mEndTime.before(d.StartTime())){	// If the time span starts after or at the ending time of this time span then these two time spans do not overlap
			//System.out.println("Does not overlap(2)");
			return false;
		}
		return true;		// Else, the time span overlaps with this time span
	}
	
	public boolean myOverlap(TimeSpan d) {
		//System.out.println("Comparing " + d.toString() + " and " + this.toString());
		if(d.StartTime().before(mStartTime) && d.EndTime().after(mEndTime)) {
			//System.out.println("my overlap");
			return true;
		}
		return false;
	}
	...
	public boolean before(TimeSpan other) {
		if(this.StartTime().before(other.StartTime())||this.StartTime().equals(other.StartTime())) {
			return true;
		}else {
			return false;
		}
	...
	public boolean after(TimeSpan other) {
		if(this.StartTime().after(other.StartTime())||this.StartTime().equals(other.StartTime())) {
			return true;
		}else {
			return false;
		}
