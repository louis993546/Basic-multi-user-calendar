	...
	public AppCellRenderer(Object value, boolean override, int row, int col,
		...
		if (col == 1 || col == 4) {
			if (colorCMD == AppList.COLORED_TITLE) {
				...
				setForeground(Color.black);
			} else if (colorCMD == AppList.COLORED) {//to be modified
				setBackground(currColor);
...
public class AppList extends JPanel implements ActionListener {
	...
	public AppList() {
		...
		tableView = new JTable(dataModel) {
			public TableCellRenderer getCellRenderer(int row, int col) {//have to modify cellCMD if appt exists
				if (col == 0 || col == 3)
	...
	public void getDataArray(Object[][] data) {
		...
		int i;
		for (i = 0; i < ROWNUM; i++) {//needs to be modified
			if (tam % 60 == 0)
	...
	private void delete() 
	{
		...
		if(apptTitle == null)
			return;
		System.out.println(apptTitle.getTitle());
		/*delete the appointment*/
		if(SingletonMediator.getBrainInstance().getThisUserName().equals(apptTitle.getUserName())) {
			ApptStorage.mAppts.remove(apptTitle.TimeSpan());//set from singleton mediator
			//repaint();
			LoginDialog.grid.repaint();
			LoginDialog.grid.applist.repaint();
			this.repaint();
		} else {
			JOptionPane.showMessageDialog(null, "You are not allowed to delete since you are not the creator of group appointment");
		}
	}
	...
	private void modify() {
		...
		if (apptTitle == null)
			return;
		if(SingletonMediator.getBrainInstance().getThisUserName().equals(apptTitle.getUserName())) {
			AppScheduler setAppDial = new AppScheduler("Modify", parent, apptTitle.getID());//set from singleton mediator
			...
			JOptionPane.showMessageDialog(null, "You are not allowed to modify since you are not the creator of group appointment");
		}
		this.repaint();
	}
	...
	private void NewAppt() {
		...
		/*if the list of locations is empty, do not allow the user to create an appointment and inform him about that*/
		...
		parent.controller.setLocationList(SingletonMediator.getBrainInstance().getAllLocations());
