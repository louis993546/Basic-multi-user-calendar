...
public class CalGrid extends JFrame implements ActionListener {
	...
	private JMenu adminMenu = new JMenu("Admin Panel");
	private JMenu locationMenu = new JMenu("Location");
	
	private JMenu dateStatus = new JMenu();
	private Clock clock;
	...
	public CalGrid(ApptStorageControllerImpl con) {
		...
		//Notification.setTimeMahcine(timeMachine);
		previousRow = 0;
		previousCol = 0;
		currentRow = 0;
		currentCol = 0;
	...
	JMenuBar createMenuBar() {
		...
		ActionListener listener = new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				else if(e.getActionCommand().equals("Location Manager")) {
					//LocationDialog l = new LocationDialog(locationController);
					locationManager = new LocationManager(controller);
				}
				else if(e.getActionCommand().equals("User Manager")) {
					userManager = new UserManager();
				}
				else if(e.getActionCommand().equals("Change Time Setting")) {
					TimeMachineDialog tm = new TimeMachineDialog(CalGrid.this);
					tm.show();
				}
				else if(e.getActionCommand().equals("Users Appointments Manager")) {
		...
		if(mCurrUser.getRole().equals("Admin")) {
			...
			adminMenu.getAccessibleContext().setAccessibleDescription("Location Management");
			mi = new JMenuItem("Location Manager");
			mi.addActionListener(listener);
			adminMenu.add(mi);
			mi = new JMenuItem("User Manager");
			mi.addActionListener(listener);
			adminMenu.add(mi);
			mi = new JMenuItem("Users Appointments Manager");
			mi.addActionListener(listener);
			adminMenu.add(mi);
		}
...
//	}
	...
	public void updateDisplay() {
		...
		displayDateLabel = today.get(today.DAY_OF_MONTH) + "-" + month + "-" + today.get(today.YEAR) + " " + today.get(today.HOUR_OF_DAY) + ":" + minute + ":" + second;
		dateStatus.setText(displayDateLabel);
