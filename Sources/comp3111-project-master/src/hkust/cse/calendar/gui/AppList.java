...
public class AppList extends JPanel implements ActionListener {
	...
	private void delete() {
		...
		/*TimeSpan newEndDate = new TimeSpan(apptTitle.TimeSpan());
		if (apptTitle.getFrequency()=="Daily"){
			newEndDate.StartTime().setDate((newEndDate.StartTime().getDate()-1));
			newEndDate.EndTime().setDate((newEndDate.EndTime().getDate()-1));
		} else if (apptTitle.getFrequency()=="Weekly"){
			newEndDate.StartTime().setDate(newEndDate.StartTime().getDate()-7);
			newEndDate.EndTime().setDate(newEndDate.EndTime().getDate()-7);
		} else if (apptTitle.getFrequency()=="Monthly"){
			newEndDate.StartTime().setMonth(newEndDate.StartTime().getMonth()-1);
			newEndDate.EndTime().setMonth(newEndDate.EndTime().getMonth()-1);
		}
		apptTitle.setTimeSpan(newEndDate);
		apptTitle.setEndDate(newEndDate);
		if (!apptTitle.TimeSpan().Overlap(apptTitle.getStartDate())) this.parent.controller.saveApptToXml(apptTitle);*/
